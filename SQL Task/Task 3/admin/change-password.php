...
<?php
    ...
    if(isset($_POST['changepassword_btn'])){
        ...
        $select_password = mysqli_query($conn, "select username from admin where username='$admin_username' and password='$current_password'");
        $row = mysqli_fetch_array($select_password);
        if($row>0){
        $update_password = mysqli_query($conn,"update admin set password='$new_password' where username='$admin_username'");
